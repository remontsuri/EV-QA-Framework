...
1. **Email** (preferred): Send a detailed report to the maintainers at the contact address listed in the repository. Use the subject line: `[SECURITY] EV-QA-Framework Vulnerability Report`.

2. **GitHub Private Vulnerability Reporting**: Use [GitHub's private vulnerability reporting feature](https://github.com/remontsuri/EV-QA-Framework/security/advisories/new) to submit a report directly.

### What to Include

- **Description**: Clear description of the vulnerability
- **Affected component**: Which module(s) are affected (e.g., `can_bus.py`, `dbc_parser.py`, `cli.py`)
- **Affected versions**: Version(s) where the issue exists
- **Steps to reproduce**: Minimal reproduction steps or proof-of-concept code
- **Impact**: What an attacker could achieve (data exposure, code execution, denial of service, etc.)
...
The `can_bus.py` module can interact with physical CAN hardware. When using this module:

- **Never** connect to vehicle CAN buses without proper authorization and safety precautions.
- The simulation mode (default) does not require physical hardware and is safe for development.
- When using `python-can` with physical interfaces, ensure the interface is properly isolated.

### DBC Parser

The `dbc_parser.py` module parses untrusted input files. When processing DBC files from external sources:
...
- The CLI and dashboard (`cli.py`, `dashboard/app.py`) accept file paths and network interfaces as arguments.
...
2. Use the simulation mode for CAN bus during development.

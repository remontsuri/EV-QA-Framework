...
def example_3_model_versioning():
    ...
    model_a.save_model(
        "models/model_a_conservative", metadata={"version": "A", "type": "conservative"}
    )
    ...
    model_b.save_model("models/model_b_tolerant", metadata={"version": "B", "type": "tolerant"})
...
def example_4_anomaly_detector_pattern():
    ...
    for batch_num in range(3):
        batch_data = (
            generate_test_telemetry_with_anomalies()
            if batch_num == 1
            else generate_normal_telemetry(10)
        )

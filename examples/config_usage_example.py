...
from ev_qa_framework import EVQAFramework, FrameworkConfig, MLConfig, SafetyThresholds
...
def example_1_default_config():
    ...
    print(
        f"Диапазон напряжения: {qa.config.safety_thresholds.min_voltage}-{qa.config.safety_thresholds.max_voltage}V"
    )
...
def example_5_comparison():
    ...
    print("-" * 60)
    print(
        f"{'Макс. температура':<30} {tesla_cfg.safety_thresholds.max_temperature:<15} {leaf_cfg.safety_thresholds.max_temperature:<15}"
    )
    print(
        f"{'Макс. напряжение':<30} {tesla_cfg.safety_thresholds.max_voltage:<15} {leaf_cfg.safety_thresholds.max_voltage:<15}"
    )
    print(
        f"{'Скачок температуры':<30} {tesla_cfg.safety_thresholds.max_temperature_jump:<15} {leaf_cfg.safety_thresholds.max_temperature_jump:<15}"
    )
    print(
        f"{'ML contamination':<30} {tesla_cfg.ml_config.contamination:<15} {leaf_cfg.ml_config.contamination:<15}"
    )
    print()

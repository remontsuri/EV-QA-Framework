...
train_data = [{"voltage": 400.0, "current": 50, "temperature": 30, "soc": 80, "soh": 95}] * 20
...
print(
    "predictions via loaded model:",
    qa_prod.ml_analyzer.model.predict(
        qa_prod.ml_analyzer.scaler.transform(df_anom[["voltage", "current", "temp"]])
    )
    if hasattr(qa_prod.ml_analyzer.model, "predict")
    else "no predict",
)

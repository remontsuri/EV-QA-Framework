...
class TestAnomalyDetection:
    ...
    def test_exact_5_degree_jump(self):
        ...
        telemetries = [
            ...
            BatteryTelemetryModel(vin=self.vin, voltage=390.0, current=50, temperature=35, soc=80, soh=98),  # Exactly 5°C
        ]
        anomalies = self.qa.detect_anomalies(telemetries)
    ...
    def test_multiple_temperature_jumps(self):
        ...
        telemetries = [
            ...
            BatteryTelemetryModel(vin=self.vin, voltage=390.0, current=50, temperature=32, soc=80, soh=98),  # -5°C (not detected, because abs change = 5 not > 5)
...
class TestNegativeScenarios:
    ...
    def test_extreme_negative_temperature(self):
        """Extreme low temperature should raise model validation error"""
    ...
    def test_extreme_high_voltage(self):
        """Extreme high voltage (within Pydantic limits, but above Warning threshold)"""
...
class TestAsyncTestSuite:
    ...
    async def test_anomaly_flag_causes_failure(self):
        ...
        results = await self.qa.run_test_suite(test_data)
        # first item remained successful, second failed due to jump
        assert results['passed'] == 1
